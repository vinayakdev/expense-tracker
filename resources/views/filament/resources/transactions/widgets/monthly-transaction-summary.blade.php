<x-filament-widgets::widget>
    ...
    <div class="relative rounded-xl border border-gray-200 bg-white p-2.5 shadow-sm dark:border-white/10 dark:bg-gray-950">
        <div class="grid gap-2 xl:grid-cols-[minmax(31rem,1fr)_minmax(40rem,42rem)] xl:items-stretch">
            <section class="flex min-w-0 items-center gap-2 rounded-lg bg-gray-50 px-2 py-2 ring-1 ring-gray-200 dark:bg-white/5 dark:ring-white/10">
                <button
                    type="button"
                    x-on:click="$dispatch('transaction-month-loading', { monthLabel: @js($this->getOlderMonthSelectionLabel()) })"
                    wire:click="showOlderMonths"
                    class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-gray-600 shadow-sm ring-1 ring-gray-200 transition hover:bg-gray-100 dark:bg-gray-950 dark:text-gray-300 dark:ring-white/10 dark:hover:bg-white/10"
                    ...
                    <div class="flex min-w-max items-center justify-start gap-1.5 py-1">
                        @foreach ($months as $month)
                            <button
                                ...
                                x-on:click="$dispatch('transaction-month-loading', { monthLabel: @js($this->getMonthSelectionLabel($month['month_key'])) })"
                ...
                <button
                    type="button"
                    x-on:click="$dispatch('transaction-month-loading', { monthLabel: @js($this->getNewerMonthSelectionLabel()) })"
                    wire:click="showNewerMonths"
                    @disabled(! $this->canShowNewerMonths())
